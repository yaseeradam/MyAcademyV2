...
<div class="space-y-6">
    ...
    <div class="flex flex-wrap gap-2">
        ...
        <a href="{{ route('settings.devices') }}"
            style="display: inline-flex; align-items: center; justify-content: center; gap: 0.5rem; border-radius: 0.75rem; padding: 0.625rem 1.25rem; font-size: 0.875rem; font-weight: 700; background: linear-gradient(135deg, #8b5cf6 0%, #7c3aed 100%); color: white; box-shadow: 0 4px 6px -1px rgba(139, 92, 246, 0.3);"
            onmouseover="this.style.transform='translateY(-2px)'; this.style.boxShadow='0 10px 15px -3px rgba(139, 92, 246, 0.4)'"
    ...
    <div class="grid grid-cols-1 gap-6 lg:grid-cols-2">
        ...
        <div
            class="rounded-3xl border-2 border-purple-200 bg-gradient-to-br from-purple-500 via-violet-600 to-indigo-600 p-6 shadow-2xl">
            <div class="flex items-center justify-between gap-3 mb-5">
                ...
                <div>
                    @if(($licenseState['ok'] ?? false) === true)
                        <span
                            class="inline-flex items-center rounded-full bg-emerald-500 px-3 py-1 text-xs font-black text-white shadow-lg gap-1">
                    ...
                    @else
                        <span
                            class="inline-flex items-center rounded-full bg-red-600 px-3 py-1 text-xs font-black text-white shadow-lg gap-1">
                            <svg class="h-3 w-3" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="3">
            ...
            <div class="space-y-4">
                <div class="rounded-2xl border-2 border-white/30 bg-white/95 backdrop-blur-sm p-4 shadow-xl">
                    ...
                    <div class="mt-2 text-sm font-semibold text-gray-900">
                        {{ $licenseState['reason'] ?? 'No license installed.' }}</div>
                ...
                <div class="rounded-2xl border-2 border-white/30 bg-white/95 backdrop-blur-sm p-4 shadow-xl">
                    ...
                    <form method="POST" action="{{ route('settings.update-license') }}" enctype="multipart/form-data"
                        class="mt-3 space-y-3">
                        @csrf
                        <input name="license" type="file" accept=".key,.json,application/json,text/plain"
                            class="w-full rounded-xl border-0 bg-white px-4 py-3 text-sm font-semibold text-gray-900 shadow-lg focus:ring-2 focus:ring-amber-500"
                            required />
                        <button type="submit"
                            class="w-full rounded-xl bg-gradient-to-r from-slate-700 to-slate-900 px-5 py-3 text-sm font-bold text-white shadow-xl hover:from-slate-800 hover:to-black transition-all flex items-center justify-center gap-2">
