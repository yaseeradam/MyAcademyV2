...
<div x-data="enhancedTable({{ $sortable ? 'true' : 'false' }}, {{ $selectable ? 'true' : 'false' }})" class="overflow-hidden rounded-2xl bg-white dark:bg-dark-100 shadow-soft ring-1 ring-gray-100 dark:ring-dark-200">
    <!-- Table Header with Search -->
    @if ($sortable || $selectable)
        <div class="border-b border-gray-100 dark:border-dark-200 p-4">
                ...
                @if ($sortable)
                    <div class="flex items-center gap-2">
                        <button class="btn-ghost text-xs" @click="sortBy('name')">
                            Sort by Name
                            <svg class="h-3 w-3 inline ml-1" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2">
    ...
    <!-- Loading Overlay -->
    <div class="absolute inset-0 bg-white/80 dark:bg-dark-100/80 flex items-center justify-center hidden" x-show="loading" x-transition>
        <div class="flex items-center gap-3">
