...
<div class="card p-6">
    <div class="mb-4">
        <h3 class="text-sm font-semibold text-gray-800 dark:text-dark-900">{{ $title ?? 'Chart' }}</h3>
        @if ($subtitle ?? false)
            <p class="text-xs text-gray-600 dark:text-dark-300 mt-1">{{ $subtitle }}</p>
        @endif
    </div>
        ...
        <div class="mt-4 flex flex-wrap gap-4 justify-center">
            @foreach ($legend as $item)
                <div class="flex items-center gap-2">
                    ...
                    <span class="text-xs text-gray-600 dark:text-dark-300">{{ $item['label'] }}</span>
                </div>
            @endforeach
        </div>
    @endif
